Feature: Pluck command available for the transient cache
  ...
  Scenario: Nested values from transient can be retrieved at any depth.
    ...
    When I run `fp transient pluck my_key foo`
    Then STDOUT should be:
      """
      bar
      """

    When I run `fp transient pluck my_key_2 foo bar`
    Then STDOUT should be:
      """
      baz
      """

    When I try `fp transient pluck unknown_key foo`
  ...
  Scenario: Nested values from site transient can be retrieved at any depth.
    ...
    When I run `fp transient pluck my_key foo --network`
    Then STDOUT should be:
      """
      bar
      """

    When I run `fp transient pluck my_key_2 foo bar --network`
    Then STDOUT should be:
      """
      baz
      """

    When I try `fp transient pluck unknown_key foo --network`
    Then STDERR should be:
      """
      Warning: Transient with key "unknown_key" is not set.
      """
